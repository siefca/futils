...
[[{:tag "comp-usage-seq" :title "Using sequential results with <code>comp</code>"}]]
^{:refer futils.named/comp :added "1.2"}
(fact

  (defn f1 [& {:as args}] (assoc args :f1 1))
  (defn f2 [& {:as args}] (concat (mapcat identity args) [:f2 2]))
  (defn f3 [& {:as args}] (assoc args :f3 3))
  
  ((named/comp f1 {:f f2 :use-seq true} f3)
   :a 1 :b 2 :c 3 :d 4 :e 5)
  => {:a 1 :b 2 :c 3 :d 4 :e 5 :f1 1 :f2 2 :f3 3}
  
  ((named/comp f1 f2 f3 {:use-seq true})
   :a 1 :b 2 :c 3 :d 4 :e 5)
  => {:a 1 :b 2 :c 3 :d 4 :e 5 :f1 1 :f2 2 :f3 3}
  
  ((named/comp f1
               {:f f2 :use-seq true}
               {:f f3 :rename-keys {:a :b :b :c}}) :a 1 :b 2)
  => {:b 1 :c 2 :f1 1 :f2 2 :f3 3}
  
  ((named/comp f1
               {:f f2 :use-seq true}
               {:f f3 :map-output true :rename-keys {:a :x}}) :a 1)
  => {:f1 1 :f2 2 :out {:f3 3 :x 1}}
  
  ((named/comp f1
               {:f f2 :use-seq true}
               {:f f3 :map-output true :post-rename {:out :result}}) :a 1)
  => {:f1 1 :f2 2 :result {:f3 3 :a 1}}
  ...
  ((named/comp f1
               ...
               {:f f3
                :map-output true
                :rename-keys {:a :x}
                :post-rename {:out :result}}
               {:use-seq true}) :a 1)
  => {:f1 1 :f2 2 :result {:f3 3 :x 1}})

[[{:tag "comp-usage-raw" :title "Using raw results with <code>comp</code>"}]]
^{:refer futils.named/comp :added "1.2"}
(fact

  (defn f1 [& {:as args}] (assoc args :f1 1))
  (defn f2 [& {:as args}] (concat (mapcat identity args) [:f2 2]))
  (defn f3 [& {:as args}] (concat (mapcat identity args) [:f3 3]))
  
  ((named/comp f1 {:f f2 :apply-raw true} {:f f3 :apply-raw true})
   :a 1 :b 2 :c 3 :d 4 :e 5)
  => {:a 1 :b 2 :c 3 :d 4 :e 5 :f1 1 :f2 2 :f3 3}
  
  ((named/comp f1 f2 f3 {:apply-raw true})
   :a 1 :b 2 :c 3 :d 4 :e 5)
  => {:a 1 :b 2 :c 3 :d 4 :e 5 :f1 1 :f2 2 :f3 3})
